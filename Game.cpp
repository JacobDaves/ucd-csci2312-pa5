...
// did manual and auto here
    ...
    bool Game::isLegal(const ActionType &ac, const Position &pos) const{
        Surroundings mySurr = getSurroundings(pos);
        int newDir;
        switch (ac) {
            case N:
                newDir = 1;
        ...
        if (mySurr.array[newDir] != INACCESSIBLE)
            return true;
        return false;
    }
        ...
        for (int i = 0; i < __grid.size(); i++){
            ...
            if(__grid[i] != nullptr){
                if(!__grid[i]->getTurned()) {
                    newDir = __grid[i]->takeTurn(getSurroundings(__grid[i]->getPosition()));
                    __grid[i]->setTurned(true);
                    if(isLegal(newDir, __grid[i]->getPosition())) {
                        newPos = move(__grid[i]->getPosition(), newDir);
